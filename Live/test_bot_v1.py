...
class DataHandler:
    ...
    def __init__(self):
        ...
        self.client.futures_change_leverage(symbol='BTCUSDT', leverage=2)
    ...
    def handle_new_date(self, new_date):
        ...
        if self.file:
            ...
            if self.position_portfolio > 0:
                self.place_order(self.close['SELL'], SIDE_SELL)
            else:
                self.place_order(self.close['BUY'], SIDE_BUY)
    ...
    def check_order(self):
        try:
            check = self.client.futures_get_order(symbol='BTCUSDT', orderId=self.order_id)
            if check['status'] == 'FILLED':
                print('Filled')
                quant = float(check['executedQty'])
                self.position_trade += self.side * quant
                self.size = 0
                self.open = True
            elif check['status'] != 'CANCELLED':
                cancel = self.client.futures_cancel_order(symbol='BTCUSDT', orderId=self.order_id)
                quant = float(cancel['executedQty'])
                self.position_trade += self.side * quant
                self.size -= quant
        except Exception as e:
    ...
    def place_order(self, price, side, order_type=ORDER_TYPE_LIMIT):
        try:
            print(f'Sending order at {self.next_timestamp}')
            print(price, side, self.size)
            order = self.client.futures_create_order(symbol='BTCUSDT',
                                                     side=side,
                                                     type=order_type,
                                                     timeInForce='GTX',
                                                     quantity=self.size,
    ...
    def handle_new_message(self, current_event_time):
        if current_event_time >= self.next_timestamp:
            if self.latest_book_ticker and self.latest_agg_trade:
                ...
                print([self.latest_book_ticker['b'], self.latest_book_ticker['B'], self.latest_book_ticker['a'],
                     self.latest_book_ticker['A'], self.latest_agg_trade['p'], self.volume])
                self.close['BUY'], self.close['SELL'] = row[0], row[2]
                if np.isnan(self.buffer[0])[0]:
                    ...
                    if not self.open and self.position_portfolio == 0:
                        if mpc_pred > self.threshold:
                            if self.signal == 0 and self.place_order(row[0], SIDE_BUY):
                                self.side = 1
                                self.signal = 1
                            elif self.signal == 1:
                                self.check_order()
                                if self.size > 0:
                                    ...
                                    self.signal = 0
                                    self.position_trade = 0
                                self.write_row(row[0], SIDE_BUY)
                            elif self.signal == -1 and self.size > 0:
                                self.check_order()
                                print(self.position_trade)
                                self.position_portfolio = self.position_trade
                                self.balance -= self.position_trade
                                ...
                                if self.position_portfolio != 0:
                                    self.open = True
                                self.write_row(row[2], SIDE_SELL)
                        elif mpc_pred < (-1 * self.threshold):
                            if self.signal == 0 and self.place_order(row[2], SIDE_SELL):
                                self.side = -1
                                self.signal = -1
                            elif self.signal == -1:
                                self.check_order()
                    ...
                    if self.open:
                        if mpc_pred > self.threshold and self.position_portfolio <= 0:
                            if self.signal == -1 and self.size > 0:
                                ...
                                self.write_row(row[2], SIDE_SELL)
                            if self.signal == 0:
                                self.size = min((2 * self.CONTRACT_SIZE), self.balance)
                                if self.place_order(row[0], SIDE_BUY):
                                    self.side = 1
                                    self.signal = 1
                            elif self.signal == 1:
                                self.check_order()
                        ...
                        elif mpc_pred < (-1 * self.threshold) and self.position_portfolio >= 0:
                            if self.signal == 1 and self.size > 0:
                                self.check_order()
                                print(self.position_trade)
                                self.position_portfolio += self.position_trade
                                ...
                                self.write_row(row[0], SIDE_BUY)
                            if self.signal == 0:
                                self.size = 2 * self.CONTRACT_SIZE
                                if self.place_order(row[2], SIDE_SELL):
                                    self.side = -1
                                    self.signal = -1
                            elif self.signal == -1:
                                self.check_order()
    ...
    def open_new_file(self):
        ...
        # Open the CSV file
        try:
            self.file = open(f'test_btc_{self.date}.csv', 'w', newline='')
