...
fn slice_to_array6(slice: &[Decimal]) -> [Decimal; 6] {
    slice.try_into().expect("slice with incorrect length")
}

async fn process_stream_data(input: &str, state: &mut HashMap<String, Decimal>) -> Result<[Decimal; 7], ()> {
...
fn convert_to_array(state: &HashMap<String, Decimal>) -> [Decimal; 7] {
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    Ok(task::spawn(open_websocket(Cow::Borrowed("btcusdt"))).await??)
}
