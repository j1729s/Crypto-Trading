...
class DataHandler:
    ...
    def __init__(self):
        ...
        self.buffer = np.full((self.BUFFER_SIZE, 9), np.nan)
    ...
    def handle_new_message(self):
        ...
        self.row = [self.latest_book_ticker['ts'], self.latest_book_ticker['bidPx'],
                                               self.latest_book_ticker['bidSz'], self.latest_book_ticker['askPx'],
                                               self.latest_book_ticker['askSz'], self.latest_book_ticker['last'],
                                               self.volume, self.buy_vol, self.sell_vol]
        print(self.row)
    ...
    def on_message(self, data):
        ...
        if data['arg']['channel'] == 'tickers':
            ...
            size = float(event_data['sz'])
            self.buy_vol += size if event_data['side'] == 'buy' else 0
            self.sell_vol += size if event_data['side'] == 'sell' else 0
            self.volume += size
    ...
    def open_new_file(self):
        ...
        # Open the CSV file
        try:
            self.file = open(f'okx_btcusdt_{self.date}.csv', 'w', newline='')
            self.writer = csv.writer(self.file)
            self.writer.writerow(['Time', 'BestBid', 'BidVol', 'BestAsk', 'AskVol', 'Price', 'Volume', 'LastBuyVol', 'LastSellVol'])
        except Exception as e:
