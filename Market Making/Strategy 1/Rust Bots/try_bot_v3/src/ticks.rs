...
pub async fn open_websocket(symbol: Cow<'static, str>, model_params: [Decimal; SIZE], tx: Sender<[Decimal; 3]>) -> Result<(), ParseError> {
                                ...
                                if let Ok(s) = result {
                                    ...
                                    match signal{
                                        1 => {
                                            ...
                                            match tx.try_send([s[0], dec!(1), s[1]]) {
                                                Ok(()) => {
                                                },
                                                Err(_) => println!("Sender error Tick: Receiver may be closed or channel is full"),
                                            }
                                        },
                                        -1 => {
                                             // Send a signal to the processing thread
                                            match tx.try_send([s[0], dec!(-1), s[3]]) {
                                                Ok(()) => {
                                                },
                                                Err(_) => println!("Sender error Tick: Receiver may be closed or channel is full"),
                                            }
