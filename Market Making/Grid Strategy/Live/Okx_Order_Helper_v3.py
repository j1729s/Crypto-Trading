...
class OrderPlacement(Process):
    ...
    def __init__(self, order_arr, order_event):
        ...
        self.bid_list = list(self.order_list[1: (GRID_SIZE + 1)])
        self.ask_list = list(self.order_list[(-1 - GRID_SIZE): -1])
    ...
    def create_order_params(self):
        ...
        first_invert = np.argmax(order_list == -1)
        new_bids = order_list[1:first_invert]
        new_asks = order_list[first_invert + 1:]
        ...
        #print("New Bids and Asks are: ", bid_list, ask_list)
...
class CancelOrders(Process):
    ...
    def create_order_list(self):
        order_list = np.copy(np.frombuffer(self.shared_orders.get_obj(), dtype=np.float32))
        first_invert = np.argmax(order_list == -1)
        self.bid_list = order_list[1:first_invert]
        self.ask_list = order_list[first_invert + 1:]
        ...
        if len(data) > 0:
            ...
            if len(params) > 20:
                with self.suppress_output():
                    x = self.Trade.cancel_multiple_orders(params[20:])
                if x['code'] == '0':
                    print(f"{20 - len(params)} Orders Cancelled")
                else:
                    print("Error while cancelling: ", x['msg'])
