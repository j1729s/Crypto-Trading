...
def open_new_file():
    ...
    try:
        ...
        params = linear_model(pd.read_csv(f"D:/Test/btcusdt_{data_date}.csv").drop('Time', axis=1), l=2, d=4)
...
"#;
    ...
    if output.status.success() {
        ...
        //let returned_values: Vec<Decimal> = parsed_values.iter().map(|&f| Decimal::from_f64_retain(f).unwrap()).collect();
        ...
        return [Decimal::default(); SIZE];
    }
}

fn main() {
    println!("Model parameters are:");
    println!("{:?}", params_arr());
}