...
from Okx_Order_Helper_v3 import OrderPlacement, CancelOrders, CheckPosition
...
class DataHandler(Process):
    ...
    async def make_grid(self):
        ...
        self.volatility = np.round(np.exp(self.volatility / 100) - 0.9, 1)
        ...
        bid_list = np.round(self.bid_price - self.bid_list * self.volatility, 1)
        ask_list = np.round(self.ask_price + self.ask_list * self.volatility, 1)

        alpha = int(np.round(self.alpha))
        if 1 < self.alpha < GRID_SIZE:
            bid_list = np.append(bid_list, np.full(alpha, self.row[1]))
            ask_list = ask_list[alpha:]
        elif -1 * GRID_SIZE < self.alpha < -1:
            ask_list = np.append(ask_list, np.full(abs(alpha), self.row[2]))
            bid_list = bid_list[abs(alpha):]
        elif self.alpha > 10:
            bid_list = np.full(GRID_SIZE * 2, self.row[1])
            ask_list = []
        elif self.alpha < -10:
            ask_list = np.full(GRID_SIZE * 2, self.row[2])
            bid_list = []

        if position > 2 * GRID_SIZE:
            bid_list = []
            ask_list = np.full(GRID_SIZE * 2, self.row[2])
        elif position < -2 * GRID_SIZE:
            ask_list = []
            bid_list = np.full(GRID_SIZE * 2, self.row[1])

        print("Ref Bid and Ask: ", bid_list, ask_list)

        self.order_list[0] = 1
        len_bid = len(bid_list)
        self.order_list[1:len_bid + 1] = bid_list
        self.order_list[len_bid+1] = -1
        self.order_list[len_bid+2:] = ask_list
