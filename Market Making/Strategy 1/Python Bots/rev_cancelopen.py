...
class CheckPosition(Thread):
    ...
    def check_position(self):
        try:
            positions = self.binance.fetchPositions(symbols=[self.symbol], params={})
            if positions is not self.position:
                self.position = positions
                print("Position is: {}".format(positions[0]['info']['positionAmt']))
        except Exception as e:
            print("Error while fetching positions", e)

    def run(self):
        schedule.every(1).seconds.do(self.check_position)
        while True:
...
class CheckOrder_Panic(Thread):
    ...
    OPEN_ORDER_THRESHOLD = 10
    CANCEL_ORDER_THRESOLD = 3
    ...
    def action_for_interval(number):
        intervals = {
            (0, 2): "Number is in the range 0 to 2",
            (3, 5): "Number is in the range 3 to 5",
            (6, 9): "Number is in the range 6 to 9",
            (10, 15): "Number is in the range 10 to 15",
        }

        for interval, message in intervals.items():
            if interval[0] <= number <= interval[1]:
                print(message)
                # Perform your desired action here based on the interval
                break
    ...
    def check_position(self):
        ...
        try:
            ...
            if len_order > self.OPEN_ORDER_THRESHOLD and len_order < 2 * self.OPEN_ORDER_THRESHOLD:
                order_ids = [open_orders[i]["id"] for i in range(3)]
                print("Cancelling")
                for idd in order_ids:
                    try:
                        self.binance.cancelOrder(idd, symbol=self.symbol, params={})
                    except Exception as e:
                        print("Error while cancelling: {}", e)
            elif len_order > 2 * self.OPEN_ORDER_THRESHOLD:
                order_ids = [open_orders[i]["id"] for i in range(len_order - self.OPEN_ORDER_THRESHOLD + 2)]
                print("Cancelling extra")
                for idd in order_ids:
                    try:
                        self.binance.cancelOrder(idd, symbol=self.symbol, params={})
                    except Exception as e:
            ...
            print("Error while checking open orders: {}", e)
        end = time.time()
        self.runtime = round(end - start)

    def run(self):
        schedule.every(self.CHECK_LAG).seconds.do(self.check_position)
        while True:
            schedule.run_pending()
            sleeptime = round((self.CHECK_LAG - self.runtime), 1) if self.runtime < 1 else 0
            sleep(sleeptime)
...
class OrderPlacement(Thread):
    ...
    def trail_order(self):
        global cancel_event, cancel_lock, order_id
