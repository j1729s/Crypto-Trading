...
class Action:
    ...
    def open_trade(self, price):
        ...
        self.rebate += self.size * self.R * abs(price)
    ...
    def trade(self, price):
        ...
        self.position = 1 if price < 0 else -1
        self.rebate += 2 * self.R * abs(price) * self.size
    ...
    def __call__(self, data):
        ...
        for index in data.index:
            ...
            if self.own:
                ...
                if index == data.index[-1]:
                    ...
                    yield {"Time": index, "Price": price, "Position": self.position, "Trade Cost": self.cost, 
                           "Volume": self.t_volume, "Profit Before TC": self.cost, "Maker Rebate": self.rebate}
            ...
            # NOT in position, BUY/SELL to OPEN
                ...
                price = sell if signal == 1 else -1*buy
...
def backtest_strategy(train_data, test_data, to_test='Pred', threshold=0.2, l=5, optimise=False, position_size=1):
    ...
    if to_test == 'Pred':
        ...
        # Formatting Data
        data["MPC"] = y_true
        data = data[(data['MPC'] == 1) | (data['MPC'] == -1)]
    ...
    return_df["Total Profit"] = return_df["Profit Before TC"] + return_df["Maker Rebate"]
    
    # Print Metrics
    if optimise == False:
        
        print("Profit before transaction cost = {} USD".format(sum(return_df["Trade Cost"])))
        print("Maker Rebate = {} USD".format(return_df.iloc[-1,5]))
