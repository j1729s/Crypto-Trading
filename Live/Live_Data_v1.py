...
class DataHandler:
    ...
    def handle_new_message(self, current_event_time):
        if current_event_time >= self.next_timestamp:
            if self.latest_book_ticker and self.latest_agg_trade:
                ...
                print([self.next_timestamp, self.latest_book_ticker['b'],
                                                   self.latest_book_ticker['B'], self.latest_book_ticker['a'],
                                                   self.latest_book_ticker['A'], self.latest_agg_trade['p'],
                                                   float(self.volume)])
                self.buffer[self.current_index] = [self.next_timestamp, self.latest_book_ticker['b'],
                                                   self.latest_book_ticker['B'], self.latest_book_ticker['a'],
                                                   self.latest_book_ticker['A'], self.latest_agg_trade['p'],
                                                   float(self.volume)]
    ...
    def open_new_file(self):
        ...
        # Open the CSV file
        try:
            self.file = open(f'btcusdt_{self.date}.csv', 'w', newline='')
